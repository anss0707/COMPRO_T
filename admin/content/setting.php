...
if (isset($_POST['simpan'])) {
    ...
    if ($row) {
        ...
        $id_setting = $row['id'];
        $update = mysqli_query($koneksi, " UPDATE settings SET email='$email', phone='$phone', logo='$logo_name', address='$address', ig='$ig', fb='$fb', x='$x', linkedin='$linkedin' WHERE id='$id_setting'");
        if ($update) {
            header("location:?page=setting&ubah=berhasil");
        }
    } else {
        //insert
        $insert = mysqli_query($koneksi, "INSERT INTO settings (email, phone, logo, address, ig, fb, x, linkedin)
        VALUES ('$email','$phone','$logo_name','$address','$ig','$fb', '$x', '$linkedin')");
        if ($insert) {
                ...
                <div class="card-body">
                    ...
                    <form action="" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-10">
                                <input type="url" name="twitter" id="" class="form-control" value="<?php echo isset($row['x']) ? $row['x'] : '' ?>">
                            </div>
                        </div>
                        <!-- Facebook -->
                        <div class="mb-3 row">
